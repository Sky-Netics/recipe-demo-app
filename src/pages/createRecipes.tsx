...
const CreateRecipes = () => {
    ...
    const saveChanges = async ()=>{
        setDisable(true);
        const response = await fetch("http://api.recipeapp.soroushsalari.com/recipes/recipes",{
            method:"POST",
            headers:{
                ...
                <div className="mx-auto w-96 sm:w-[500px] md:w-[600px] lg:w-[1000px] lg:flex lg:justify-between bg-white px-16 sm:px-20 py-10 rounded-md shadow-2xl">    
                    <div>
                        <div className="mt-3 w-full">
                            ...
                            <div><input onChange={(e)=>{setNewRecipes({...newRecipes,title:e.target.value})}} className="border border-black rounded w-full h-9" type="text" id="name" name="name" /></div>
                        </div>
                        <div className="flex gap-2">
                            <div className="mt-3 w-full relative">
                                <div><label htmlFor="category">Category</label></div>
                                <div>
                                    <input onChange={(e)=>{setNewRecipes({...newRecipes,category:e.target.value})}} className="border border-black rounded w-full h-9" list="categorylist" id="category" name="category" />
                            ...
                            <div className="mt-3 w-full">
                                ...
                                <div>
                                    <input onChange={(e)=>{setNewRecipes({...newRecipes,people_served:+e.target.value})}} list="servinglist" className="border border-black rounded w-full h-9" id="serving" name="serving" />
                        ...
                        <div className="flex gap-2">
                            <div className="mt-3 w-full">
                                ...
                                <div>
                                    <input onChange={(e)=>{setNewRecipes({...newRecipes,country:e.target.value})}} list="countrylist" className="border border-black rounded w-full h-9" id="country" name="country" />
                            ...
                            <div className="mt-3 w-full">
                                ...
                                <div>
                                    <input onChange={(e)=>{setNewRecipes({...newRecipes,rating:+e.target.value})}} list="rating" className="border border-black rounded w-full h-9" id="rate" name="rate" />
                                    <datalist id="rating">
                                        <option value="1"/>
                                        <option value="2"/>
                                        <option value="3"/>
                        ...
                        <div className="mt-3 w-full">
                            ...
                            <div><input onChange={(e)=>{setNewRecipes({...newRecipes,cooking_time:e.target.value})}} className="border border-black rounded w-full h-9" type="text" id="timing" name="timing" /></div>
                        </div>
                    </div>
                    <div>
                        <div className="mt-5">
